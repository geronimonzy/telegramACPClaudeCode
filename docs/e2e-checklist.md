...
- [ ] `systemctl --user restart telegram-acp-bridge` (or an equivalent process
      restart): all persisted sessions reattach on boot with their history.
...
- [ ] The General-topic `message_thread_id` assumption is verified: messages
      ...
      `threadId === undefined`, and only `/new` is accepted there — everything
      else gets the "this is the General topic" notice.
